...
const DropDown = ({ setSettingStore , settingStore }) => {
    ...
    const createNewItemHandler = () => {
        ...
        else if(tempCreateItem.text && tempCreateItem.value){
            setSettingStore('options' , [...(settingStore?.options || []) , tempCreateItem]);
        }
        setTempCreateItem(DEFAULT_TEMP_NEW_FORM);
    ...
    const checkCreateTriggerRender = () => {
        if(isCreatorOpened) {
            if(tempCreateItem.text && tempCreateItem.value) {
                return (
                    <div>
                        <div>
                            <TickIcon />
    ...
    const deleteOptionHandler = optionValue => {
        setSettingStore("options" , settingStore.options.filter(item => item.value !== optionValue))
    }
            ...
            <div className="dropDownElement__itemDirectory">
                {
                    settingStore?.options?.map((option , index) => (
                        <div key={index} className="dropDownElement__item">
                            ...
                            <div onClick={() => deleteOptionHandler(option.value)} className="dropDownElement__item__controller">
                                <TrashIcon />
                            </div>
                ...
                <div className="dropDownElement__formForCreate__form">
                    <div>
                        <Input 
                            ...
                            value={tempCreateItem.value}
                            label="مقدار"
                            onChange={value => changeTempCreateStore("value" , value)} />
                    </div>
                    <div>
                        <Input 
                            inputStyle={{ backgroundColor : "transparent" }} 
                            value={tempCreateItem.text} 
                            label="عنوان" 
                            onChange={value => changeTempCreateStore("text" , value)} />
                    </div>
                </div>
            </div>
