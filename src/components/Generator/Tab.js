...
const Tab = ({
    ...
    const openCreatedSettingHandler = option => {
        ...
        const { config : { type , title , defaultValue }, name , id } = option;
        setSelectedSettingType(type);
        
        setSettingStore({ defaultValue , name , title , id , _config : option.config })
    }
    ...
    const updateSettingHandler = newData => editOption(newData)
